<template>
  <div v-if="value">
    <template v-if="adding">
      <div class="dvs-flex">
        <fieldset class="dvs-fieldset dvs-mb-4 dvs-mr-4">
          ...
          <input v-model="newMeta.attribute_name" type="text" />
        </fieldset>

        <fieldset class="dvs-fieldset dvs-mb-4 dvs-mr-4">
          <label>Attribute Value</label>
          <input v-model="newMeta.attribute_value" type="text" />
        </fieldset>

        <fieldset class="dvs-fieldset dvs-mb-4">
          <label>Content</label>
          <input v-model="newMeta.content" type="text" />
        </fieldset>
      </div>
      ...
      <button
        class="dvs-btn dvs-btn-primary dvs-text-xs dvs-mb-10"
        :disabled="isInvalid"
    ...
    <button v-if="!adding" class="dvs-btn dvs-btn-primary dvs-text-xs" @click="adding = true">
      Add New Meta Tag
    </button>
  </div>
</template>
