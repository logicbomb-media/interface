<template>
  <admin-container v-if="languages.data.length">
    <template v-slot:message>
      When you add a language to this site it is immediately enabled. Afterwards you can create
      translated versions of pages that will be linked to one another allowing you to provide ways
      to switch languages on your front-end. We
      <a
        href="https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes"
        class="dvs-font-bold"
        target="_blank"
      >
        Highly suggest using the ISO 639-1 2 letter codes
      </a>
      but you can technically use whatever you want.
    </template>
    <template v-slot:content>
      <div class="dvs-flex dvs-items-end dvs-mt-20 dvs-mb-6">
        <fieldset class="dvs-fieldset dvs-mr-8 dvs-flex-grow">
          ...
          <input v-model="newLanguage.code" type="text" maxlength="2" class="dvs-w-full" />
        </fieldset>
        ...
        >
          ...
          >
            ...
            <fieldset class="dvs-fieldset">
              <input v-show="language.editCode" v-model="localValue.data[key].code" type="text" />
            </fieldset>
          </div>
